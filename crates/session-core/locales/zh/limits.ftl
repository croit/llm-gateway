...
limits-intro = 限制调用方在滑动时间窗口内可使用的请求数、令牌数或花费金额。规则按最具体优先解析：用户自身的规则优先，否则取其角色中最宽松的，否则使用全局默认值。若无任何规则，则所有人都不受限制。仅计入计费池（自托管且 enforce_limits = false 的池不计入），且用户的整个额度在其 API 令牌、聊天和计划任务之间共享。
limits-add-heading = 添加或更新限制
limits-field-subject = 适用于
limits-field-subject-id = 角色 / 用户
limits-field-subject-id-ph = 角色 id 或用户邮箱
...
limits-missing-subject-id = 请输入角色 id 或用户邮箱
