...
impl Client {
    ...
    /// (so wiremock tests can inject their own). Not `#[cfg(test)]`-gated
    /// because the ComfyUI *tool* tests live in `gateway-runtime`, a different
    /// crate — a `cfg(test)` build of this crate is a separate crate instance
    /// whose types wouldn't unify with theirs. Unused in production builds.
