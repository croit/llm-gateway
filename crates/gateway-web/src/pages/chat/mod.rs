...
async fn render_chat_response(
    ...
    let (effort, capabilities) = if read_only {
        (
            gateway_core::server::reasoning::Effort::Standard,
            Vec::new(),
        )
    } else {
        let effort = gateway_core::server::reasoning::Effort::from_db(
            gateway_core::server::db::chat_session_settings::get_effort(&state.db, &active.id)
                .await
                .ok()
                .flatten()
                .as_deref(),
    ...
    let compacted_up_to_seq =
        gateway_core::server::db::chat_compactions::get(&state.db, &active.id)
            .await
            .ok()
            .flatten()
            .map(|c| c.up_to_seq);
    ...
    match kind {
        CapKind::Skill => {
            if target == ToolState::On {
                gateway_core::server::db::chat_session_skills::record(&state.db, session_id, key)
                    .await
            } else {
                gateway_core::server::db::chat_session_skills::remove(&state.db, session_id, key)
                    .await
            }
        }
        CapKind::Tool => match target {
            ToolState::On => {
                gateway_core::server::db::chat_session_tools::set(
                    &state.db, session_id, key, true, "user",
                )
                .await
            }
            ToolState::Off => {
            ...
            ToolState::Auto => {
                gateway_core::server::db::chat_session_tools::clear(&state.db, session_id, key)
                    .await
            }
        },
    }
...
pub async fn chat_effort_set(
    ...
    let effort = gateway_core::server::reasoning::Effort::from_db(Some(&form.effort));
    if let Err(err) = gateway_core::server::db::chat_session_settings::set_effort(
        &state.db,
        &session_id,
        effort.as_str(),
    )
...
async fn build_capabilities(
    ...
    for entry in crate::pages::entries_for_roles(state, &user.roles) {
    ...
    if let Ok(connected) =
        gateway_core::server::db::user_mcp::connected_keys(&state.db, &user.id).await
    {
        for ck in connected {
            let Ok(Some(c)) = gateway_core::server::db::mcp_catalog::get(&state.db, &ck).await
            else {
                continue;
            };
            if !c.enabled {
