...
async fn try_run_action(
    state: &Arc<RamaState>,
    action: &ScheduledAction,
    user: &gateway_core::server::db::users::User,
) -> Result<(String, String), super::DbError> {
    ...
    crate::server::headless::drive(
        ...
        crate::server::headless::DriveParams {
            ...
            source: gateway_core::server::db::usage::UsageSource::Scheduled,
            history_limit,
        },
    )
    .await;
...
mod tests {
    ...
    async fn fresh_db() -> gateway_core::server::db::Pool {
        gateway_core::server::db::open(std::path::Path::new(":memory:"))
            .await
            .unwrap()
    }

    async fn seed_user(pool: &gateway_core::server::db::Pool, id: &str) {
        let now = Timestamp::now();
