...
| `fluent-templates` | `session-core` | UI i18n (`session_core::i18n`). `static_loader!` embeds every locale's `.ftl` files into the binary at compile time (no runtime filesystem access) and gives the `Loader::lookup`/`try_lookup`/`try_lookup_with_args` API the `t`/`t_args` wrappers call. Chosen over simpler flat-map i18n crates (e.g. `rust-i18n`) because Fluent correctly handles locale plural/grammar rules — notably Russian's `one`/`few`/`many`/`other` split, which a plain `HashMap<key, String>` can't express. |
| `fluent-bundle` | `session-core` | Not re-exported by `fluent-templates`, so pulled in directly to construct `FluentValue`/the args map `t_args` takes for interpolated/pluralised messages (`i18n::args(...)`). Same version `fluent-templates` depends on internally — pinned together in `[workspace.dependencies]` so they can't drift. |
...
| `pdfium-render` | `gateway` | Optional `mode="images"` escalation tier for the `fetch_attachment` PDF reader: rasterises scanned-PDF pages for a vision model when the text layer is empty. The one runtime native-library exception to the static-binary rule (loads Chromium's `pdfium`, BSD-3, dynamically). See [Notes](#notes-on-specific-deps). |
